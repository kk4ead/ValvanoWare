...
void DMA_Transfer(volatile uint32_t *source, uint8_t *destination, uint32_t count){ 
  ucControlTable[CH8]   = (uint32_t)source;                 // first and last address
  ucControlTable[CH8+1] = (uint32_t)destination+count-1;    // last address
